...
public enum AvroSchema: Codable, Hashable, Sendable {
    ...
    public init() { self = .unknownSchema(UnknownSchema("")) }
 
    // MARK: - Schema lookup
 
    func findSchema(name: String) -> AvroSchema? {
        switch self {
        case .recordSchema(let s):
            return s.findSchema(name: name)
        case .unionSchema(let s):
            return s.branches.lazy.compactMap { $0.findSchema(name: name) }.first
        case .enumSchema(let s):
            return s.symbols.contains(name) ? self : nil
        default:
    ...
    public func isBytes()     -> Bool { if case .bytesSchema   = self { return true }; return false }
    ...
    public func isInteger() -> Bool {
        if case .intSchema  = self { return true }
        if case .longSchema = self { return true }
        return false
    }
    ...
    public func isByte() -> Bool {
        ...
        if case .fixedSchema = self { return true }
        return false
    }
    ...
    func getEnumSymbols() -> [String]    { guard case .enumSchema(let e)  = self else { return [] };  return e.symbols }
    func getEnumIndex(_ v: String) -> Int? { guard case .enumSchema(let e) = self else { return nil }; return e.symbols.firstIndex(of: v) }
    func getMapAttribute() -> MapSchema?  { guard case .mapSchema(let a)  = self else { return nil }; return a }
    func getInt()          -> IntSchema?  { guard case .intSchema(let s)  = self else { return nil }; return s }
    func getLong()         -> IntSchema?  { guard case .longSchema(let s) = self else { return nil }; return s }
    func getBytes()        -> BytesSchema? { guard case .bytesSchema(let s) = self else { return nil }; return s }
    func getFixedSize()    -> Int?        { guard case .fixedSchema(let s) = self else { return nil }; return s.size }
    func getArrayItems()   -> AvroSchema? { guard case .arraySchema(let s) = self else { return nil }; return s.items }
    func getMapValues()    -> AvroSchema? { guard case .mapSchema(let s)   = self else { return nil }; return s.values }
    func getUnionList()    -> [AvroSchema]{ guard case .unionSchema(let s) = self else { return [] };  return s.branches }
    func getField()        -> FieldSchema?  { guard case .fieldSchema(let s) = self else { return nil }; return s }
    func getRecord()       -> RecordSchema? { guard case .recordSchema(let s) = self else { return nil }; return s }
    func getError()        -> ErrorSchema?  { guard case .errorSchema(let s) = self else { return nil }; return s }
...
extension AvroSchema {
    ...
    /// Handles cross-type resolution when reader and writer schemas differ.
    ...
    mutating func resolvingDifferent(from schema: AvroSchema) throws -> Bool {
        // Reader is a union: match its first branch against the writer.
        ...
        // Writer is a union: match the reader against the first matching writer branch.
        if case .unionSchema(let writerUnion) = schema {
            guard let idx = writerUnion.branches.firstIndex(of: self) else {
                throw AvroSchemaResolutionError.SchemaMismatch
            }
